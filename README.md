...
A collection of useful Python functions that have been built to fufill a variety of purposes.
...
This function takes in a IP subnet mask, and it computes the corresponding numeric block size (or the subnet mask bit length).
...
Performs a bitwise AND operation between a string IP address and a string subnet mask, returning resultant string. This returns the Network Address in IP addressing.
...
## `psucceed.py`, `pfail.py`, `pbad.py`, `pwarn.py`, `pcolor.py`
...
phelp("This program is great for testing things",
      [('--help','Prints out help for this program'),('-c','Compile something')])
